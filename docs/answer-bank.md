...
full name, preferred name, email, phone and common text address fields. They
apply to text controls in known general/contact/applicant/Workday identity
sections only. Normalization ignores case, whitespace
...
When a user manually enters a final value and leaves the field, an unknown exact
question gets a site-scoped canonical field with `review` policy. That records
the value without claiming that differently worded questions mean the same thing.
...
`application-autofill/extension` is an ATS-independent unpacked Chrome/Edge
extension for HTTPS application pages. It is injected only after the user clicks
the popup, so it does not receive standing access to browsing history. There is
no local server, startup entry, background process or pairing token. An ignored
`local-profile.json` seeds the extension's private `chrome.storage.local` on its
first run. ATS-specific custom widgets use adapters within this one extension;
...
- `site` reuses it only on the current HTTPS origin.
- `position` reuses it only on the current origin and canonical job path or
  detected requisition ID.
...
Opening the popup also remembers nonempty values already selected on the page.
Afterward, trusted `blur` and `change` events remember final values; an
Eightfold listbox selection is captured after its option click. Individual
keystrokes are not recorded, and
synthetic events generated by the extension are ignored. Exact unknown questions
become site-scoped review fields; they never become safe-fill fields merely by
being observed. A different canonical answer is reported as a conflict rather
than overwritten. Existing text, choice and radio values are never overwritten.
...
Native text, textarea, number, select and radio controls are supported. The
Micron/Eightfold listbox combobox is recognized; it is filled only after its
open option list contains one exact match. Its saved answers require review and
cannot be given global reuse scope. Other custom comboboxes stay manual.
...
workstation answer bank. It is not part of normal use. New answers recorded by
the extension stay in that browser profile; they are not copied back into the
Python answer bank automatically.
